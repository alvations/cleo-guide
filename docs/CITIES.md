...
| Singapore &amp; Southeast Asia (int'l · **pastel light/dark** · **one page per place**) | `Singapore/` (43 place pages + `Singapore/index.html` hub; `Singapore/toa-payoh.html` = just Toa Payoh) | `data/singapore.dataset.json` | `data/singapore-research/` | 163 | live · 43 town/city pages built by `tools/build-singapore-pages.py`; opens each place on its own pins; 20 UNVERIFIED restaurant pins pending helper; 4 closed flagged; per-city sources kept separate. Rebuild: `python3 tools/rebuild-city.py singapore --build`. |
...
| Wheeling WV + National Road corridor | `cities/wheeling.html` | `data/wheeling.dataset.json` | `data/wheeling-research/` | building | 5 areas (Wheeling/Washington PA/Ohio Valley/Cambridge/Zanesville → Columbus edge); discovery in progress. Rebuild: `python3 tools/rebuild-city.py wheeling-wv --build`. |
