...
| Singapore &amp; Southeast Asia (int'l · **pastel light/dark** · **one page per place**) | `Singapore/` (43 place pages + `Singapore/index.html` hub; `Singapore/toa-payoh.html` = just Toa Payoh; `Singapore/ho-chi-minh-city.html` = HCMC) | `data/singapore.dataset.json` | `data/singapore-research/` | 253 | live · 43 town/city pages built by `tools/build-singapore-pages.py`; opens each place on its own pins. **2 live/clickable:** Toa Payoh (56) + Ho Chi Minh City (63 pins: 34 sights + 29 food) — rest greyed while populated (`LIVE_SLUGS`). ~60 UNVERIFIED restaurant pins pending helper; closed flagged; per-city sources kept separate (shared institutional labels pinned generic in `consolidate.py`). **City vs town tier + NYC-style metro districts/outskirts are data-driven in [`data/metros.json`](../data/metros.json) — see [docs/METROS.md](METROS.md) to give the next city the same treatment.** Rebuild: `python3 tools/rebuild-city.py singapore --build`. |
