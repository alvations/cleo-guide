...
| Rhineland — Cologne · Bonn · Düsseldorf (DE) | `cities/rhineland.html` (linked from the Germany hub) | `data/rhineland.dataset.json` | `data/rhineland-research/` | 216 | live · lower-Rhine triangle on ONE map, 3 areas: **KOLN** (Cologne), **BONN**, **DUS** (Düsseldorf). **218 discovered (wave 1), all ≥2-credible or lone Michelin/UNESCO; 216 rendered** (126 sights + 90 food — Cologne 98, Düsseldorf 74, Bonn 44). **All sights geocoded via Wikidata** (127/127, 1 held — Bonner Kirschblüte) + food via Michelin/heritage structured geo (KOLN 43/43, BONN+DUS 54/55). Beer/Brauhaus (Kölsch + Altbier), Little-Tokyo, and a deep Michelin bench (Ox & Klee 2★, Le Moissonnier, Im Schiffchen). 2 closures flagged (Kaspars, Brauerei zum Schiffchen — the latter worth a status re-check). 7 Bonn single-source food held in `_held_bonn_single_source.json` for wave-2 corroboration. `tools/density.py rhineland`: KOLN +1 · DUS +5 · BONN +12 to the ~287 benchmark. Rebuild: `python3 tools/rebuild-city.py rhineland --build`. |
| Aachen & the Dreiländereck (Euregio Maas-Rhein · DE/NL/BE) | `cities/aachen.html` (linked from the country hub as 🇩🇪 Aachen & the Dreiländereck) | `data/aachen.dataset.json` | `data/aachen-research/` | 203 | live · tri-border, 5 areas: **AACHEN** (city — UNESCO cathedral, thermal springs, RWTH/Pontviertel), **STADT** (StädteRegion — Monschau, Kornelimünster, Stolberg), **EIFEL** (National Park + Düren/Jülich), **NL** (Maastricht + Dutch South Limburg), **BE** (Ostbelgien — Eupen, Kelmis, Hautes Fagnes). **178 discovered, all ≥2-credible or a lone institutional authority; 148 rendered** (113 sights + 35 food — food by area: NL 13 · EIFEL 7 · STADT 7 · AACHEN 4 · BE 4). **All 113 sights geocoded** (Wikidata landmarks — 74/74 DE + 39/39 NL/BE, 0 unverified). Sourced in DE/NL/FR (Aachener Zeitung/Nachrichten, WDR, Route Aachen, Eifel Tourismus, Visit Maastricht, De Limburger, Grenz-Echo, BRF, Michelin/UNESCO/Gault&amp;Millau). 4 closures flagged (Chocolaterie Jacques museum, Thermenmuseum Heerlen, Felsenkeller, Take One). ~30 restaurants held UNVERIFIED for the browser geocode-helper (WebSearch can't surface their venue decimals) — never town-centroid faked. Rebuild: `python3 tools/rebuild-city.py aachen --build`. |
| Antwerp (BE) | `cities/antwerp.html` (linked from the 🇧🇪 Belgium hub) | `data/antwerp.dataset.json` | `data/antwerp-research/` | 137 | live · 2 areas: **ANT** (centre — Cathedral, MAS, Zuid, Zurenborg, Diamond District), **ANTR** (Mechelen, Lier, Berchem, Deurne, the Port). **138 discovered (2 waves), all ≥2-credible native (Dutch) or lone Michelin/UNESCO/Gault&amp;Millau; 137 rendered** (59 sights + 78 food). Beer a first-class grouped layer (`cz:["Beer"]` → BEER cuisine): De Koninck Bolleke, 't Waagstuk, Antwerpse Brouw Compagnie/Seef, Het Anker/Gouden Carolus (Mechelen). Deep Michelin bench (Hertog Jan 2★, Bloesem, Fine Fleur). 2 closures flagged (Kulminator, De Vagant); Oriental Delight held. Patine held UNVERIFIED for the helper. Built off the shared `tools/belgium_build.py` + `tools/belgium_consolidate.py`. Rebuild: `python3 tools/rebuild-city.py antwerp --build`. |
| Ghent / Gent (BE) | `cities/ghent.html` (linked from the 🇧🇪 Belgium hub) | `data/ghent.dataset.json` | `data/ghent-research/` | 108 | live · 2 areas: **GEN** (centre — Gravensteen, the Ghent Altarpiece, Graslei, Patershol, SMAK), **GENR** (Sint-Martens-Latem, the Leie, Deinze). **110 discovered, all ≥2-credible native or lone institutional; 108 rendered** (53 sights + 55 food). Canon: waterzooi, Gentse stoverij, cuberdon/neuzekes, Tierenteyn mustard, Gruut city brewery, veggie-capital dining; Michelin bench (Vrijmoed 2★). Beer grouped (10). 2 closures flagged (Souvenir, Frituur De Gouden Saté); Sensum/Ribas held UNVERIFIED (address mismatch). Rebuild: `python3 tools/rebuild-city.py ghent --build`. |
| Brussels / Bruxelles / Brussel (BE) | `cities/brussels.html` (linked from the 🇧🇪 Belgium hub) | `data/brussels.dataset.json` | `data/brussels-research/` | 139 | live · bilingual (FR+NL), 2 areas: **BRU** (centre & communes — Grand-Place, Sablon, Marolles, Ixelles, Saint-Gilles, EU quarter, Matongé), **BRUR** (Uccle, Schaerbeek, Anderlecht/Cantillon, Tervuren, Waterloo). **140 discovered (2 waves), all ≥2-credible or lone institutional; 139 rendered** (71 sights + 68 food). Canon: moules-frites, carbonnade/stoemp, gaufre, chocolate; **lambic/gueuze/kriek** (Cantillon, Moeder Lambic) grouped as beer (16); Michelin bench (Comme Chez Soi, La Paix, Le Chalet de la Forêt 2★). En Stoemelings flagged CLOSED (bankrupt 2024, brand-only); La Paix kept open (relocating); Sahbaz held single-source (`_held_single_source.json`). Rebuild: `python3 tools/rebuild-city.py brussels --build`. |
| Bruges / Brugge (BE) + the coast | `cities/bruges.html` (linked from the 🇧🇪 Belgium hub) | `data/bruges.dataset.json` | `data/bruges-research/` | 123 | live · 2 areas: **BRG** (UNESCO centre — Markt & Belfort, Holy Blood, Groeninge, Memling, Begijnhof, canals, De Halve Maan), **BRGR** (the coast — Damme, Ostend, Knokke/Zwin, Zeebrugge, Nieuwpoort, De Panne). **125 discovered (2 waves), all ≥2-credible or lone institutional; 123 rendered** (63 sights + 60 food). Canon: Brugse Zot, Belgian chocolate, garnaalkroketten, North-Sea fish, the UNESCO horseback shrimp fishers; beer grouped (11), SEA layer 18, deep Michelin bench incl. the Knokke cluster (Bartholomeus 2★). Mu.ZEE (Ostend) flagged CLOSED (renovation to ~2028); Sincfala/Brouwerij Stene held UNVERIFIED. Rebuild: `python3 tools/rebuild-city.py bruges --build`. |

**Belgium region scope & structure:** four SEPARATE dark-engine dataset cities (each `city + surrounding area`),
grouped under the hand-authored 🇧🇪 **Belgium hub** (`Belgium/index.html`) — the `dataset-cities` country kind
(see [docs/COUNTRIES.md](COUNTRIES.md)). All four share `tools/belgium_consolidate.py` (one Belgian cuisine
taxonomy incl. **Beer** as a first-class grouped layer + Dutch/French/English sight-collection keywords) and
`tools/belgium_build.py`; each `data/<city>-research/consolidate.py` and `tools/build-<city>.py` is a thin
wrapper. Sourced in Dutch (Flemish) and French; Yelp/TripAdvisor/RateBeer/Untappd count ZERO toward the
≥2-credible bar. 507 rendered places across the four maps.
