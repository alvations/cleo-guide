#!/usr/bin/env python3
# Build cities/sanfrancisco.html from the Cleveland engine + the San Francisco dataset.
# Data comes from data/sanfrancisco.dataset.json; coordinates are injected from
# data/geocodes.json cities["san-francisco-ca"] (the build FAILS on any missing/UNVERIFIED pin).
import re, os, json
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC  = os.path.join(ROOT, "cleveland.html")
OUT  = os.path.join(ROOT, "cities", "sanfrancisco.html")
DS   = json.load(open(os.path.join(ROOT, "data", "sanfrancisco.dataset.json"), encoding="utf-8"))
...
_REG = json.load(open(os.path.join(ROOT, "data", "geocodes.json"), encoding="utf-8"))["cities"].get("san-francisco-ca", {})
def _has_pin(n):
    e = _REG.get(n)
...
_GEO = json.load(open(os.path.join(ROOT, "data", "geocodes.json"), encoding="utf-8"))["cities"]["san-francisco-ca"]
def _decode(raw):
...
if _lats:
    ...
    _clat, _clng, _zoom = 37.76, -122.44, 12          # empty-registry fallback only
...
# map backdrop labels → San Francisco municipalities (drop the Lake Erie polygon; keep neutral labels)
