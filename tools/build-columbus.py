#!/usr/bin/env python3
# Build cities/columbus.html from the Cleveland engine + the Columbus dataset.
# Data comes from data/columbus.dataset.json; coordinates are injected from
# data/geocodes.json cities["columbus-oh"] (the build FAILS on any missing/UNVERIFIED pin).
import re, os, json
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC  = os.path.join(ROOT, "cleveland.html")
OUT  = os.path.join(ROOT, "cities", "columbus.html")
DS   = json.load(open(os.path.join(ROOT, "data", "columbus.dataset.json"), encoding="utf-8"))
...
_REG = json.load(open(os.path.join(ROOT, "data", "geocodes.json"), encoding="utf-8"))["cities"].get("columbus-oh", {})
def _has_pin(n):
    e = _REG.get(n)
...
_GEO = json.load(open(os.path.join(ROOT, "data", "geocodes.json"), encoding="utf-8"))["cities"]["columbus-oh"]
def _decode(raw):
...
if _lats:
    ...
    _clat, _clng, _zoom = 39.9612, -82.9988, 12          # empty-registry fallback only
...
for a, b in [("cle_trip","col_trip"),("cle_seen","col_seen"),("cle_gkey","col_gkey"),
             ("cleveland-my-list","columbus-my-list"),("cleveland-field-guide","columbus-field-guide")]:
    new = new.replace(a, b)
# legend swatches
...
# map backdrop labels → Columbus municipalities (drop the Lake Erie polygon; keep neutral labels)
...
rep("<title>Cleveland Field Guide — 130 Places, Sourced</title>",
    "<title>Columbus Field Guide — Sourced</title>")
rep('<p class="eyebrow">Field guide · every place from all seven sources</p>',
    '<p class="eyebrow">Field guide · Columbus &amp; the Peninsula, sourced</p>')
rep('<h1>Cleveland<span class="thin">the complete odd &amp; overlooked</span></h1>',
    '<h1>Columbus<span class="thin">the city &amp; the Peninsula &mdash; icons, neighborhoods &amp; famous food</span></h1>')
