#!/usr/bin/env python3
# Build cities/erie.html from the Cleveland engine + the Erie PA / I-79 corridor dataset.
# Region = Erie, PA (Bayfront, Presque Isle, downtown) + the Lake Erie wine belt (North East) + the
# I-79/US-19 corridor south toward Pittsburgh (Meadville, Edinboro, Grove City, Conneaut Lake, Harmony,
# Cranberry). Standard engine theme. Coordinates injected from geocodes.json["erie-pa"]; build FAILS on
# any missing/UNVERIFIED pin. Areas with no gated+geocoded place are dropped (so the map only shows what's live).
import re, os, json
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC  = os.path.join(ROOT, "cleveland.html")
OUT  = os.path.join(ROOT, "cities", "erie.html")
DS   = json.load(open(os.path.join(ROOT, "data", "erie.dataset.json"), encoding="utf-8"))
...
for a, b in [("cle_trip","er_trip"),("cle_seen","er_seen"),("cle_gkey","er_gkey"),
             ("cleveland-my-list","erie-my-list"),("cleveland-field-guide","erie-field-guide")]:
    new = new.replace(a, b)
...
rep("<title>Cleveland Field Guide — 130 Places, Sourced</title>",
    "<title>Erie, PA &amp; the Lake Erie Corridor Field Guide — Sourced</title>")
rep('<p class="eyebrow">Field guide · every place from all seven sources</p>',
    '<p class="eyebrow">Field guide · Erie &amp; the Lake Erie corridor, sourced</p>')
rep('<h1>Cleveland<span class="thin">the complete odd &amp; overlooked</span></h1>',
    '<h1>Erie<span class="thin">the Bayfront, the wine belt &amp; the I-79 corridor to Pittsburgh</span></h1>')
rep('<p class="standfirst">143 sights and 40 places to eat, each traceable to the source that named it. <strong>Switch modes below</strong> &mdash; food lives on its own map so it never clutters the sightseeing one. Tick the box on anything to build your own list, then export it to Google or Apple Maps.</p>',
    '<p class="standfirst">%d sights and %d places to eat across <strong>Erie, PA</strong> &mdash; the Bayfront and Presque Isle &mdash; the <strong>Lake Erie wine belt</strong> at North East, and the <strong>I-79/US-19 corridor</strong> south toward Pittsburgh through Meadville, Edinboro and Grove City. Each is traceable to the source that named it. Erie’s canon is its own: the <strong>Greek-sauce hot dog</strong>, <strong>Stanganelli’s</strong> pepperoni balls, <strong>sponge candy</strong>, Great-Lakes perch, and the wineries &amp; breweries — plus Presque Isle, the U.S. Brig Niagara and the Bicentennial Tower. <strong>Switch modes below</strong>, filter by <strong>area</strong> or <strong>collection</strong>, and tick anything to build your own list, then export it to Google or Apple Maps.</p>' % (nP, nF))
new = re.sub(r'<meta name="description"[^>]*>',
    '<meta name="description" content="Erie PA &amp; the Lake Erie / I-79 corridor field guide — %d sights and %d places to eat across Erie, North East wine country, Meadville, Edinboro and Grove City, each traceable to its source (Erie Times-News, Erie Reader, Visit Erie, PA DCNR, Atlas Obscura), on one interactive map with area, collection and cuisine filters, a trip builder and exports.">' % (nP, nF), new)
new = new.replace(", Cleveland OH", ", Erie PA")
new = new.replace(">Cleveland — my list<", ">Erie — my list<").replace(">Cleveland — my list<", ">Erie — my list<")
new = new.replace('placeholder="witchcraft, waterfall, chess, kielbasa…"', 'placeholder="Greek dog, sponge candy, Presque Isle, perch…"')
new = new.replace("? 'laksa, dim sum, pastrami, cannoli…' : 'witchcraft, waterfall, chess, kielbasa…'",
                  "? 'Greek dog, pepperoni ball, sponge candy, perch…' : 'Presque Isle, Brig Niagara, Bicentennial Tower, wineries…'")
new = new.replace('href="index.html" style="color:var(--bone-dim)', 'href="../index.html" style="color:var(--bone-dim)')

new = new.replace("last verified 2026-08-08", "last verified 2026-08-28")
new = new.replace(
...
'''  <span style="opacity:.8">Web-researched and fact-checked via the pipeline (data/sources.json, docs/SOURCES.md): sourced across the Erie Times-News (GoErie), Erie Reader, Visit Erie, the Meadville Tribune, PA DCNR &amp; NPS official sites, Uncovering PA and Atlas Obscura. Every coordinate is verified into data/geocodes.json and every place status-checked open. A handful of newly-added spots are pending a final coordinate pass before they appear.</span><br><br>''')

ER_APPENDIX = (
 "+ '<div class=\"srcrow\"><span class=\"k\">FOOD RULES</span><div class=\"t\">How the cuisine filters were policed'\n"
 "  + '<span>Every food card names a specific dish — a cuisine label alone doesn’t qualify. Erie’s canon is its own: the <b>Greek-sauce hot dog</b> (New York Lunch, Sara’s), deep-fried <b>pepperoni balls</b> (Stanganelli’s), <b>sponge candy</b> (Romolo, Pulakos), Federal Hill <b>Italian</b>, Great-Lakes <b>perch &amp; walleye</b> fish fries, and the <b>Lake Erie wine belt</b> (Mazza, Penn Shore, Arrowhead), sourced to the Erie Times-News and Erie Reader. A cuisine tag names the kitchen’s own tradition, never one dish it happens to serve.</span></div></div>'\n"
 "  + '<div class=\"srcrow\"><span class=\"k\">HOW SOURCED</span><div class=\"t\">Web-searched and fact-checked'\n"
 "  + '<span>Every place is traceable to a credible source — the Erie Times-News/GoErie, Erie Reader, Visit Erie, the Meadville Tribune, PA DCNR, NPS, Uncovering PA, Atlas Obscura and official sites — recorded in data/sources.json. Every coordinate is verified into data/geocodes.json and every place status-checked open. Yelp/TripAdvisor never count toward the two-source bar.</span></div></div>';"
)
