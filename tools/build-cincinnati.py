#!/usr/bin/env python3
# Build cities/cincinnati.html from the Cleveland engine + the Cincinnati dataset.
# Data comes from data/cincinnati.dataset.json; coordinates are injected from
# data/geocodes.json cities["cincinnati-oh"] (the build FAILS on any missing/UNVERIFIED pin).
import re, os, json
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC  = os.path.join(ROOT, "cleveland.html")
OUT  = os.path.join(ROOT, "cities", "cincinnati.html")
DS   = json.load(open(os.path.join(ROOT, "data", "cincinnati.dataset.json"), encoding="utf-8"))
...
_REG = json.load(open(os.path.join(ROOT, "data", "geocodes.json"), encoding="utf-8"))["cities"].get("cincinnati-oh", {})
def _has_pin(n):
    e = _REG.get(n)
...
_GEO = json.load(open(os.path.join(ROOT, "data", "geocodes.json"), encoding="utf-8"))["cities"]["cincinnati-oh"]
def _decode(raw):
...
if _lats:
    ...
    _clat, _clng, _zoom = 39.1030, -84.5120, 12          # empty-registry fallback only
...
for a, b in [("cle_trip","cin_trip"),("cle_seen","cin_seen"),("cle_gkey","cin_gkey"),
             ("cleveland-my-list","cincinnati-my-list"),("cleveland-field-guide","cincinnati-field-guide")]:
    new = new.replace(a, b)
# legend swatches
...
# map backdrop labels → Cincinnati municipalities (drop the Lake Erie polygon; keep neutral labels)
...
rep("<title>Cleveland Field Guide — 130 Places, Sourced</title>",
    "<title>Cincinnati Field Guide — Sourced</title>")
rep('<p class="eyebrow">Field guide · every place from all seven sources</p>',
    '<p class="eyebrow">Field guide · Cincinnati, sourced</p>')
rep('<h1>Cleveland<span class="thin">the complete odd &amp; overlooked</span></h1>',
    '<h1>Cincinnati<span class="thin">the Queen City &mdash; chili, Over-the-Rhine &amp; the Ohio riverfront</span></h1>')
rep('<p class="standfirst">143 sights and 40 places to eat, each traceable to the source that named it. <strong>Switch modes below</strong> &mdash; food lives on its own map so it never clutters the sightseeing one. Tick the box on anything to build your own list, then export it to Google or Apple Maps.</p>',
    '<p class="standfirst">%d sights and %d places to eat across Cincinnati and the Northern Kentucky riverfront, each traceable to the source that named it. The city’s food canon is its own: <strong>Cincinnati chili</strong> (Skyline, Camp Washington), goetta, Montgomery Inn ribs, Findlay Market, Graeter\u2019s ice cream and the German brewing heritage of Over-the-Rhine. <strong>Switch modes below</strong>, filter by <strong>neighborhood</strong> or <strong>collection</strong>, and tick anything to build your own list, then export it to Google or Apple Maps.</p>' % (nP, nF))
new = re.sub(r'<meta name="description"[^>]*>',
    '<meta name="description" content="Cincinnati field guide — %d sights and %d places to eat across Cincinnati and Northern Kentucky, each traceable to its source (the Cincinnati Enquirer, Cincinnati Magazine, CityBeat, James Beard, Atlas Obscura), on one interactive map with neighborhood, collection and cuisine filters, a trip builder and exports.">' % (nP, nF), new)
new = new.replace(", Cleveland OH", ", Cincinnati OH")
new = new.replace(">Cleveland — my list<", ">Cincinnati — my list<").replace(">Cleveland — my list<", ">Cincinnati — my list<")
new = new.replace('placeholder="witchcraft, waterfall, chess, kielbasa…"', 'placeholder="chili, goetta, Union Terminal, ribs…"')
new = new.replace("? 'laksa, dim sum, pastrami, cannoli…' : 'witchcraft, waterfall, chess, kielbasa…'",
                  "? 'chili, goetta, ribs, Graeter’s…' : 'Union Terminal, the zoo, Roebling, Music Hall…'")
...
'''  <span style="opacity:.8">Web-researched and fact-checked via the pipeline (data/sources.json, docs/SOURCES.md): sourced across James Beard, the Cincinnati Enquirer, Cincinnati Magazine, CityBeat, Cincinnati Refined, Soapbox, Atlas Obscura and official / visitor-bureau sites. Every coordinate is verified into data/geocodes.json and every place status-checked open. A handful of newly-added spots are pending a final coordinate pass before they appear.</span><br><br>''')
...
SV_APPENDIX = (
 ...
 "  + '<span>Every food card names a specific dish — a cuisine label alone doesn’t qualify. Cincinnati’s signature strength is a food canon all its own: <b>Cincinnati chili</b> (Skyline, Gold Star, Camp Washington \u2014 3-way, 4-way, 5-way and coneys), <b>goetta</b>, <b>Montgomery Inn</b> ribs, <b>Findlay Market</b>, <b>Graeter\u2019s</b> ice cream, <b>LaRosa\u2019s</b> pizza and the German brewing heritage of <b>Over-the-Rhine</b>, reborn at <b>Rhinegeist</b> and <b>MadTree</b>. Every food card names a specific dish, sourced to the Cincinnati Enquirer, Cincinnati Magazine, CityBeat and James Beard. A cuisine tag names the kitchen\u2019s own tradition, never one dish it happens to serve. A cuisine tag names the kitchen’s own tradition, never one dish it happens to serve.</span></div></div>'\n"
